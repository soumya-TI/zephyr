...
static int spi_ti_unicomm_configure(const struct device *dev, const struct spi_config *config)
{
	...
	/* CTL1.MSB = 1: MSB first. Set unless caller requests LSB first. */
...
static int spi_ti_unicomm_init(const struct device *dev)
{
	...
	/*
	 * Safe reset defaults: Motorola 4-wire, Mode 0, 8-bit, controller
	 * mode, MSB first, no parity.  CPOL/CPHA and SCR are overwritten by
	 * spi_ti_unicomm_configure() on the first transfer.
	 */
	UPDATE_REG(cfg->unicomm_spi_base + UNICOMM_SPI_CTL0,
		   SPI_CTL0_FRF_MOTOROLA_4WIRE | SPI_CTL0_DSS_8BIT,
		   UNICOMMSPI_CTL0_FRF_MASK | UNICOMMSPI_CTL0_SPO_MASK | UNICOMMSPI_CTL0_SPH_MASK |
			   UNICOMMSPI_CTL0_DSS_MASK);

	UPDATE_REG(cfg->unicomm_spi_base + UNICOMM_SPI_CTL1,
		   UNICOMMSPI_CTL1_CP_MASK | UNICOMMSPI_CTL1_MSB_MASK,
		   UNICOMMSPI_CTL1_PES_MASK | UNICOMMSPI_CTL1_PREN_MASK |
			   UNICOMMSPI_CTL1_PTEN_MASK | UNICOMMSPI_CTL1_MSB_MASK |
			   UNICOMMSPI_CTL1_CP_MASK | UNICOMMSPI_CTL1_LBM_MASK);
	...
	LOG_INF("SPI controller init done for 0x%08x", cfg->unicomm_spi_base);
				     ...
				     const struct spi_buf_set *rx_bufs)
{
	...
	/*
	 * Assert CS now and hold it for the entire transfer. Hardware auto-CS
	 * would deassert between FIFO bursts, which resets the display's
	 * internal byte counter mid-pixel and garbles the output. Software CS
	 * (via spi_context) keeps the line low until we explicitly release it.
	 */
	...
	while (spi_context_tx_buf_on(ctx) || spi_context_rx_buf_on(ctx)) {
		...
		/*
		 * Fill the TX FIFO with as many bytes as it will accept before
		 * waiting for any RX data.  Keeping the FIFO continuously fed
		 * prevents the hardware from deasserting CS between bytes, which
		 * would otherwise split every multi-byte write (e.g. 16-bit
		 * RGB565 pixels) into separate one-byte transactions and corrupt
		 * the pixel stream seen by the display controller.
		 *
		 * For RX-only transfers we clock out dummy 0x00 bytes so the
		 * peripheral can shift data in.
		 */
		while ((spi_context_tx_buf_on(ctx) || spi_context_rx_buf_on(ctx)) &&
		       !(sys_read32(base + UNICOMM_SPI_STAT) & UNICOMMSPI_STAT_TXFF_MASK)) {
		...
		/* Drain the corresponding RX bytes */
