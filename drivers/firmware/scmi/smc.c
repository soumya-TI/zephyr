...
#define SCMI_SMC_CHAN_NAME(proto, idx) \
...
#define SCMI_SMC_CHAN_DEFINE(node_id, proto, idx)			\
	...
	DT_SCMI_TRANSPORT_CHAN_DEFINE(node_id, idx, proto,		\
				    &(SCMI_SMC_CHAN_NAME(proto, idx)));	\

 
#define DT_INST_SCMI_BASE_CHAN_DEFINE(inst)			\
	BUILD_ASSERT(DT_INST_NODE_HAS_PROP(inst, shmem),		\
		     "SMC transport must have shmem phandle");		\
															\
	BUILD_ASSERT(DT_INST_NODE_HAS_PROP(inst, arm_smc_id),			\
		     "SMC transport must have arm,smc-id");			\
															\
	SCMI_SMC_CHAN_DEFINE(DT_INST(inst, DT_DRV_COMPAT), 		\
...
static int scmi_smc_send_message(const struct device *transport,
...
static int scmi_smc_read_message(const struct device *transport,
				  struct scmi_channel *chan,
				  struct scmi_message *msg)
{
	struct scmi_smc_channel *smc_chan;
...
static bool scmi_smc_channel_is_free(const struct device *transport,
				      struct scmi_channel *chan)
{
	struct scmi_smc_channel *smc_chan = chan->data;

	/* Per SCMI spec: Bit[0]=1 means FREE, Bit[0]=0 means BUSY */
	return scmi_shmem_channel_status(smc_chan->shmem) &
...
DT_INST_SCMI_SMC_DEFINE(0, PRE_KERNEL_1,
