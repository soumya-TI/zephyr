...
{
  MATH_Vec2 Iab_A;              //!< Alpha/beta current values, A
  MATH_Vec2 Vab_V;              //!< Alpha/beta voltage values, V
  float32_t dcBus_V;            //!< DC bus voltage value, V
  float32_t speed_ref_Hz;       //!< Speed reference value, Hz
} EST_InputData_t;
...
{
  float32_t angle_rad;          //!< Estimated rotor flux angle at t = n+1, rad
  float32_t fe_rps;             //!< Electrical frequency estimate, rad/sec
  float32_t fe_Hz;              //!< Electrical frequency estimate, Hz
  float32_t fm_lp_rps;          //!< Low pass filtered mechanical frequency estimate, rad/sec
  float32_t oneOverDcBus_invV;  //!< DC bus voltage inverse, 1/V
} EST_OutputData_t;
