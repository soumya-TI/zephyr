...
void motor_control_init(const char * asset_path)
{
     ...
     * foc_display_bridge_init() calls motor_digits_init() internally (which
     * motor_sim_init() used to do) and creates the 100 ms lv_timer_t that
     * reads motorVars_M1 and calls motor_sim_publish() with live data. */
