...
lv_obj_t * surface_create(lv_obj_t * parent)
{
    ...
    if (ti_thermostat_check_target(TI_THERMOSTAT_TARGET_ALL)) {
        ...
        lv_obj_set_name_static(card_0, "surface_#");
        lv_obj_set_width(card_0, CONST_CONTENT_W);
        lv_obj_set_height(card_0, LV_SIZE_CONTENT);
        lv_obj_set_style_pad_all(card_0, CONST_SPACE_12, 0);

        lv_obj_add_style(card_0, &style_card_surface, 0);
